...
int vetor_save(Vetor* v, char* nomeArquivo){
    FILE *outfile;
    outfile = fopen (nomeArquivo, "wb");
    fwrite(&(v->length),sizeof(int),1,outfile);
    for(int i=0;i<v->length;i++){
        fwrite(&(v->vetor[i]), sizeof(v->vetor[i]), 1, outfile);
    }
...
Vetor* vetor_load(char* nomeArquivo){
    ...
    DataType num;
    int length;
    infile = fopen (nomeArquivo, "rb");
    if (infile != NULL){
        fread(&length, sizeof(int), 1, infile);
        for(int i=0;i<length;i++){
            fread(&num, sizeof(DataType), 1, infile);
            vetor_add(vetor,num);
        }
    }
    return vetor;
